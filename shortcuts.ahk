...
{
    TerminalExecutable := "%USERPROFILE%\AppData\Local\Microsoft\WindowsApps\wt.exe"
    TerminalDescriptor := "ahk_exe WindowsTerminal.exe"
}
Else If (Terminal = "WezTerm")
{
    TerminalExecutable := "%USERPROFILE%\scoop\apps\wezterm-nightly\current\wezterm-gui.exe"
    TerminalDescriptor := "ahk_exe wezterm-gui.exe"
}
Return
...
;; SSH や Docker 先の Vim でもいい感じにするため
~Esc::
    IME_SET(0)
Return
...
;; jk で Esc
;; これはクッソ破壊的だが Vim を使うには便利になるだろう
; ~j::
;     Input, OutputVar, L1 T0.5
;     If (OutputVar = "k")
;     {
;         Send, {Backspace}{Esc}
;         IME_SET(0)
...
;; ランチャー
#Enter::Launch(TerminalExecutable, TerminalDescriptor)
#+i::Launch("C:\Program Files\Google\Chrome\Application\chrome.exe", "ahk_exe chrome.exe")
#+m::Launch("C:\Program Files\Mozilla Thunderbird\thunderbird.exe", "ahk_exe thunderbird.exe")
#+o::Launch("C:\Program Files\WindowsApps\NAVER.LINEwin8_7.14.1.0_x86__8ptj331gd3tyt\LINEAPP.exe", "ahk_exe lineapp.exe")
#+e::Launch("C:\Program Files\WindowsApps\Evernote.Evernote_10.50.10.0_x64__q4d96b2w5wcc2\app\Evernote.exe", "ahk_exe Evernote.exe")
